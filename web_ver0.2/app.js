...
function renderFavPage() {
  ...
  const favPlaces = mergedData.filter(p => favorites.includes(p.place_id));

  // 일정 요약 카드 (이벤트 있는 즐겨찾기만)
  const withEvents = favPlaces.filter(p => p.ev && p.ev.title);

  if (withEvents.length > 0) {
    scheduleCard.innerHTML = withEvents.slice(0, 3).map(pl => {
      const ev = pl.ev;
      ...
      if (startDate && today < startDate) {
        badgeType = 'booking';
        badgeText = '예매 예정';
        dateText  = `예매 시작: ${ev.start_date}`;
      } else if (endDate) {
        dateText = `관람 기간: ~ ${ev.end_date}`;
      }

      return `
        <div class="fav-schedule-item" data-id="${pl.place_id}">
          <div class="fav-schedule-icon">${emoji}</div>
          <div class="fav-schedule-info">
            <div class="fav-schedule-name">${pl.name} — ${ev.title}</div>
            <div class="fav-schedule-date">
              <i class="fa-regular fa-calendar"></i>${dateText || ev.start_date || '일정 정보 없음'}
            </div>
          </div>
    ...
    // 일정 카드 클릭 시 상세 오픈
    scheduleCard.querySelectorAll('.fav-schedule-item').forEach(item => {
      item.addEventListener('click', () => openDetail(item.dataset.id));
    });
  } else {
  ...
  favList.innerHTML = favPlaces.map((pl, idx) => {
    const isFav = true;
    ...
    return `
      <div class="place-item" data-id="${pl.place_id}" role="button" tabindex="0" aria-label="${pl.name} 상세 보기">
        <span class="place-item__rank${idx < 3 ? ' top' : ''}">${idx + 1}</span>
        <div class="place-item__info">
          <div class="place-item__name">${pl.name}</div>
          <div class="place-item__meta">
            <span class="place-item__addr">${pl.address || '-'}</span>
            <span class="place-item__cat">${pl.category || ''}</span>
            <span class="crowd-dot ${pl.crowd}"></span>
          </div>
          <div class="place-item__badges">${pills}</div>
        </div>
        <div class="place-item__thumb">
          ${emoji}
          <button class="fav-btn active" data-pid="${pl.place_id}" aria-label="즐겨찾기 해제">
  ...
  favList.querySelectorAll('.place-item').forEach(item => {
    item.addEventListener('click', () => openDetail(item.dataset.id));
    item.addEventListener('keydown', e => { if (e.key === 'Enter') openDetail(item.dataset.id); });
  });
}
...
function switchToHome() {
  ...
  document.getElementById('search-bar-wrap').hidden && null; // 검색바 상태 유지
...
function updateTicker() {
  ...
  // 즐겨찾기된 장소 중 이벤트 있는 것들
  let tickerItems = [];
  const favPlaces = mergedData.filter(p => favorites.includes(p.place_id) && p.ev && p.ev.title);

  if (favPlaces.length > 0) {
    tickerItems = favPlaces.map(pl => {
      const ev = pl.ev;
      const today = new Date();
      const startDate = ev.start_date ? new Date(ev.start_date) : null;
      let prefix = '📅';
      ...
      if (startDate && today < startDate) {
        prefix = '🎟️';
        dateStr = ` | 예매 시작: ${ev.start_date}`;
      } else if (ev.end_date) {
        dateStr = ` | 관람 기간: ~ ${ev.end_date}`;
      }
      return { text: `${prefix} ⭐ ${pl.name} — ${ev.title}${dateStr}`, placeId: pl.place_id };
    });
  } else {
    // 즐겨찾기 없으면 전체 이벤트 최신 3건
    const recentEvents = mergedData
      .filter(p => p.ev && p.ev.title)
      .slice(0, 3);
    if (recentEvents.length > 0) {
      tickerItems = recentEvents.map(pl => ({
        text: `📅 ${pl.name} — ${pl.ev.title}${pl.ev.end_date ? ` (~${pl.ev.end_date})` : ''}`,
        placeId: pl.place_id,
      }));
    } else {
      tickerItems = [{ text: '⭐ 장소에 즐겨찾기를 추가하면 일정 알림이 여기에 표시됩니다', placeId: null }];
    }
  }

  // 무한 스크롤을 위해 2번 반복
  const itemsHTML = tickerItems.map(item =>
    `<span class="ticker-item" data-pid="${item.placeId || ''}">${item.text}</span>`
  ).join('');

  track.innerHTML = itemsHTML + itemsHTML; // 2x for seamless loop

  // 클릭 시 상세 오픈
  track.querySelectorAll('.ticker-item[data-pid]').forEach(el => {
  ...
  // 텍스트 길이에 따라 애니메이션 속도 조절
  const totalChars = tickerItems.reduce((sum, i) => sum + i.text.length, 0);
  const duration = Math.max(15, Math.min(45, totalChars * 0.4));
...
function openShareSheet() {
  ...
  if (favPlaces.length === 0) {
    ...
    desc.textContent = `즐겨찾기 장소 ${favPlaces.length}곳을 친구에게 공유해 보세요!`;
  }
...
function buildShareText() {
  const favPlaces = mergedData.filter(p => favorites.includes(p.place_id));
  if (favPlaces.length === 0) return '주말해 앱에서 AI 맞춤 여가 장소를 추천받아 보세요! 🌿';

  const list = favPlaces.slice(0, 5).map((pl, i) => `${i + 1}. ${pl.name} (${pl.category || '장소'})`).join('\n');
  return `🌿 주말해 — 내 즐겨찾기 장소\n\n${list}${favPlaces.length > 5 ? `\n외 ${favPlaces.length - 5}곳 더...` : ''}\n\n👉 주말해 앱에서 확인: ${window.location.origin}/web/index.html`;
}

// 카카오톡 공유
function shareKakao() {
  const text = buildShareText();
  // Kakao JS SDK 없이 카카오 딥링크로 폴백
  const kakaoUrl = `kakaolink://send?text=${encodeURIComponent(text)}`;
  const webFallback = `https://sharer.kakao.com/talk/friends/picker/link?app_key=DEMO&text=${encodeURIComponent(text)}`;
  // 딥링크 시도 후 실패하면 웹 공유
  ...
  try {
    a.click();
    setTimeout(() => {
      // 앱이 없으면 Web Share 또는 클립보드로
      if (navigator.share) {
        navigator.share({ title: '주말해 즐겨찾기', text });
      } else {
...
function shareAllonebank() {
  const text = buildShareText();
  // 올원뱅크 딥링크 (앱 스킴)
  const alloneUrl = `allonebank://share?text=${encodeURIComponent(text)}`;
  const a = document.createElement('a');
  a.href = alloneUrl;
  try {
    a.click();
    setTimeout(() => {
      // 앱 미설치 시 클립보드
      copyToClipboard(text, '올원뱅크 앱이 없습니다. 클립보드에 복사되었습니다.');
    }, 1200);
  } catch {
...
function shareInstagram() {
  const text = buildShareText();
  // Instagram은 직접 텍스트 공유가 제한적 — 스토리 딥링크 또는 Web Share
  ...
  try {
    a.click();
    setTimeout(() => {
      // 앱 없거나 실패 시 클립보드 복사 후 안내
      copyToClipboard(text, 'Instagram을 열었습니다! 스토리 작성 시 클립보드를 붙여넣어 주세요.');
    }, 1200);
  } catch {
    copyToClipboard(text, 'Instagram 앱이 없습니다. 클립보드에 복사되었습니다.');
  }
}
...
// 클립보드 복사 + 토스트 표시
function copyToClipboard(text, message = '링크가 클립보드에 복사되었습니다!') {
  const toast = document.getElementById('copy-toast');
  navigator.clipboard.writeText(text).then(() => {
    toast.querySelector ? (toast.innerHTML = `<i class="fa-solid fa-circle-check"></i> ${message}`) : null;
...
function shareMore() {
  ...
  if (navigator.share) {
    navigator.share({
      title: '🌿 주말해 — 내 즐겨찾기 장소',
      text,
      url: `${window.location.origin}/web/index.html`,
    }).catch(err => console.warn('Share cancelled:', err));
  } else {
    copyToClipboard(text, '이 기기에서는 공유 기능이 지원되지 않아 클립보드에 복사되었습니다.');
  }
}
...
function initEvents() {
  ...
  document.getElementById('bnav-home')?.addEventListener('click', () => {
    if (currentView !== 'home') switchToHome();
  });

  // ★ 하단 내비게이션 — 즐겨찾기
  document.getElementById('bnav-fav')?.addEventListener('click', () => {
    if (currentView !== 'fav') switchToFav();
  });
